...
const mainfunction = function (search) {
  fetch('https://api.pexels.com/v1/search?query=' + search, {
    headers: {
      Authorization: 'btGbPR2T4ut63nA0Skmuh9WujIzlPz9QgAQmSNMWYAjOvvAPQbjp3TqE',
    },
  })
    .then((response) => {
      if (response) {
    ...
    .then((data) => {
      ...
      for (let i = 0; i < data.photos.length; i++) {
        ...
        cardsTitles[i].addEventListener('click', function () {
          location.assign(
            `details.html?imgId=${data.photos[i].id}&search=${search}`
          );
        });
